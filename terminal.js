  ...
  function actualAdventure() {
    ...
    document.addEventListener('keydown', (event) => {
      if(event.keyCode == 8) {
        ...
        if(input.length > 0) {
          input = input.substr(0, highlight - 1).concat(input.substr(highlight + 1, input.length));
          highlight--;
        }
      }
      ...
      if(highlight >= input.length - 1) {
        output = con.concat(dir, input, "<div id=\"highlighted\" class=\"highlighted\"> &nbsp&nbsp&nbsp </div>");     
      }
